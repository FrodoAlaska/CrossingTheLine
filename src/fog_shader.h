...
inline nikola::GfxShaderDesc get_fog_shader() {
  return nikola::GfxShaderDesc {
    ...
    "\n"
    "uniform vec3 u_view_pos, u_ambient;"
    ...
    "  vec3 fog_factor       = 1 - vec3(linearize_depth(gl_FragCoord.z)) + vec3(linearize_depth(gl_FragCoord.z));"
    "\n"
    "  frag_color = vec4(vec3(point_lights_factor + dir_light_factor) * fog_factor, 1.0);"
    "}"
    "\n"
    "vec3 directional_light(DirectionalLight light, vec4 diffuse_texel, vec4 specular_texel) {"
